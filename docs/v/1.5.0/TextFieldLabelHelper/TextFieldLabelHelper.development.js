...
function TextFieldLabelHelper(_ref) {
  var name = _ref.name,
      ...
      props = _babel_runtime_helpers_objectWithoutProperties__WEBPACK_IMPORTED_MODULE_1___default()(_ref, ["name", "placeholder", "className", "label", "helper", "htmlFor", "labelId", "state", "style", "disabled", "value", "required", "inputRef"]);
  ...
  var disabledState = disabled ? 'disabled' : '';
  var classes = ['rex-text-field-label-helper', className, stateClass, disabledState].filter(function (singleClass) {
    return singleClass && singleClass.length > 0;
  }).join(' ').trim();
